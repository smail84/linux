...
static void pblk_gc_line(struct work_struct *work)
{
...
int pblk_gc_run(struct pblk *pblk)
{
	...
	while (run_gc && !list_empty(&l_mg->closed_list)) {
		...
		line = list_first_entry(&l_mg->closed_list, struct pblk_line,
									list);
	...
	list_for_each_entry_safe(line, tline, &gc_list, list) {
		...
		INIT_WORK(&line_ws->ws, pblk_gc_line);
