...
/* write completion context - rename to c_ctx*/
...
#define PBLK_KICK_SECTS 16
	int write_cnt;		/* Counter for pblk_write_kick */
...
enum {
	...
	/* Line state */
	PBLK_LINESTATE_FREE = 3,
	PBLK_LINESTATE_OPEN = 4,
	PBLK_LINESTATE_CLOSED = 5,
	PBLK_LINESTATE_GC = 6,
	PBLK_LINESTATE_BAD = 7,
};
...
struct pblk_line {
	...
	unsigned int sec_in_line;	/* Number of valid secs in line */
	...
	unsigned int left_eblks;	/* Blocks left for erasing */
	unsigned int left_seblks;	/* Blocks left for sync erasing */

	unsigned int left_msecs;	/* Sectors left for mapping */
	unsigned int left_ssecs;	/* Sectors left to sync */
	unsigned int cur_sec;		/* Sector map pointer */
	unsigned int isc;		/* Invalid sector count in line */
...
struct pblk_line_mgmt {
	...
	struct list_head closed_list;	/* Full lines closed. Avail. for GC */
...
struct pblk {
	...
	// struct pblk_w_luns w_luns; //JAVIER
...
#define pblk_w_rq_size (sizeof(struct nvm_rq) + sizeof(struct pblk_compl_ctx))
...
void pblk_line_run_ws(struct pblk *pblk, struct pblk_line *line,
...
void pblk_bio_free_pages(struct pblk *pblk, struct bio *bio, int off,
			 int nr_pages);
int pblk_update_map_cache(struct pblk *pblk, sector_t laddr,
			  struct ppa_addr ppa);
int pblk_update_map_dev(struct pblk *pblk, sector_t laddr,
			  struct ppa_addr new_line, struct ppa_addr entry_line);
int pblk_update_map_gc(struct pblk *pblk, sector_t laddr, struct ppa_addr ppa,
...
int pblk_gc_run(struct pblk *pblk);
